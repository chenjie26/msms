<template>
    <section>
        ...
        <el-col :span="24" class="toolbar">
            <el-form :inline="true" :model="formInline" class="demo-form-inline">
                <el-form-item>
                    <el-select v-model="formInline.service_id" @change="serviceChange" placeholder="请选择分类">
                        <el-option
 ...
 @current-change="handleCurrentChange" :current-page="1" :page-sizes="[10, 20, 30, 40]" :page-size="10" layout="total, prev, pager, next, jumper"
        ...
        <!--编辑界面-->
        <el-dialog :title="editFormTtile" v-model="editFormVisible" :close-on-click-modal="false" size="small" >
            <el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
			<el-form-item label="分类" >
				<el-select v-model="editForm.service_id" placeholder="请选择分类">
					<el-option
					  v-for="item in formInline.options"
					  :label="item.name"
					  :value="item.id">
				...
				<el-form-item label="价格" prop="price">
                    <el-input placeholder="请输入内容" v-model="editForm.price"></el-input>
                </el-form-item>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
                ordersDetailUrl:function (obj){
                    return 'http://shama.jcjever.com/member_service_details?page=1&active_id='+obj.id;
                },
                DetailUrl: (obj) => {
                    if(obj.id){
                        return 'http://shama.jcjever.com/service_details/'+obj.id;
                    }else{
                        return 'http://shama.jcjever.com/service_details';
                    }
                },
                ListUrl: function (obj){
					...
					if(!obj.service_id)
                        obj.service_id = '';
                    return 'http://shama.jcjever.com/service_details?page=' + obj.page + '&keywords=' + obj.keywords + '&service_id=' + obj.service_id;
                },
                ListServiceUrl: function (obj){
                    if(!obj.page)
                        obj.page = 1;
                    if(!obj.keywords)
                        obj.keywords = '';
                    return 'http://shama.jcjever.com/services?page=' + obj.page + '&keywords=' + obj.keywords;
                },
                formInline: {
                    keywords: '',
                    options:[],
    ...
    methods: {
            serviceChange(val){
                this.loadData({page:1,service_id:val});
            },
            search(){
            ...
            loadServiceData(obj){
                this.$http.get(this.ListServiceUrl(obj)).then((response) => {
            ...
            handleEdit:function(row){
				...
				this.editForm.service_id = row.service_id;
            },
            //编辑 or 新增
            editSubmit:function(){
                var _this=this;
                            ...
                            setTimeout(function(){
                                ...
                                if(0 == _this.editForm.id){
                                    _this.insert({
										...
										service_id:_this.editForm.service_id,
										price:_this.editForm.price,
                                    },function(response){
                                        _this.tableData.push({
                                            id:response.body.service_detail.id,
											name:_this.editForm.name,
		                                    phone:_this.editForm.phone,
											desc:_this.editForm.desc,
											service_id:_this.editForm.service_id,
											price:_this.editForm.price,
											created_at:response.body.service_detail.created_at
                                        })
                                    });
                                }else{
                                    _this.update({
                                ...
                                if(_this.editForm.id==0){
                                    ...
                                    for(var i=0;i<_this.tableData.length;i++){
                                        if(_this.tableData[i].id==_this.editForm.id){
											...
											_this.tableData[i].price=_this.editForm.price;
                                            break;
                                        }
                                    }
                                }
            ...
            handleAdd:function(){
				...
				_this.editForm.name = '',
				_this.editForm.phone = '',
				_this.editForm.desc = '',
				_this.editForm.service_id = '',
