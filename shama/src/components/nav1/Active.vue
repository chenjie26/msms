	...
	<section>
		...
		<template>
			<el-table :data="tableData" highlight-current-row v-loading="listLoading" style="width: 100%;">
				...
				<el-table-column prop="_message" label="备注" :formatter="formatText" sortable>
				</el-table-column>
				...
				<el-table-column inline-template :context="_self" label="操作" width="180">
					<span>
						<el-button type="text" size="small" @click="handleDetail(row)">报名列表</el-button>
    ...
    data() {
      return {
      			ordersDetailUrl:function (obj){
      				return 'http://shama.jcjever.com/member_actives?page=1&active_id='+obj.id;
      			},
				DetailUril: (obj) => {
					if(obj.id){
						return 'http://shama.jcjever.com/actives/'+obj.id;
					}else{
						return 'http://shama.jcjever.com/actives';
					}
				},
	  			ListUrl: function (obj){
	  				...
	  				if(!obj.keywords)
	  					obj.keywords = '';
	  				return 'http://shama.jcjever.com/actives?page=' + obj.page + '&keywords=' + obj.keywords;
								...
								if(0 == _this.editForm.id){
									_this.insert({
										...
										_this.tableData.push({
											id:response.body.active.id,
											name:_this.editForm.name,
											status:_this.editForm.status,
											message:_this.editForm.message,
											created_at:response.body.active.updated_at
										})
									});
								}else{
